...
export default function Bet() {
  ...
  // AQUI A MUDANÇA: O loadGames reage ao loggedUser para garantir que tudo esteja pronto
  useEffect(() => {
    if (loggedUser) {
      loadGames();
    }
  ...
  async function handleBet() {
    if (!loggedUser) return; // Segurança extra
    ...
    await updateDoc(doc(db, "users", loggedUser.id), {
      balance: loggedUser.balance - Number(stake),
    });
                  ...
                  <div className="bg-white rounded-3xl shadow-lg p-6 border border-gray-100">
                    ...
                    <div className="space-y-4 mb-6">
                      {[2.5, 4.5, 6.5, 8.5, 10.5].map(line => (
                        <div key={line} className="grid grid-cols-2 gap-2">
                          <button 
                            onClick={() => setSelectedMarket({type: 'over', line, odd: getOdd(selectedPlayer.category, line, 'over'), label: `Mais de ${line} pts`})}
                            className="bg-green-50 text-green-700 py-3 rounded-xl font-bold text-xs"
                          >
                            Over {line} ({getOdd(selectedPlayer.category, line, 'over').toFixed(2)})
                          </button>
                          <button 
                            onClick={() => setSelectedMarket({type: 'under', line, odd: getOdd(selectedPlayer.category, line, 'under'), label: `Menos de ${line} pts`})}
                            className="bg-blue-50 text-blue-700 py-3 rounded-xl font-bold text-xs"
                          >
                            Under {line} ({getOdd(selectedPlayer.category, line, 'under').toFixed(2)})
                          </button>
                        </div>
                      ))}
                    </div>
                    ...
                    {selectedMarket && (
                      <div className="bg-gray-50 rounded-2xl p-5 border-t-4 border-blue-500">
                        ...
                        <button onClick={handleBet} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold">Confirmar Aposta</button>
                      </div>
                    )}
                  </div>
                )}
